...
1. **Phase 1 — RAG 查询**：将测试问题逐条发送给 RAG 系统（当前生产链路基于 OpenAI Agents SDK 的 `plan -> loop -> output` 流程），获取实际回答。
...
测试用例存放在 `tools/eval/test_cases.json`，当前版本 v3.0.0，共 120 条。
...
评测脚本为 `tools/eval/run_eval.py`，支持多种运行模式：
...
# 按内容分类筛选
python run_eval.py --category safety

# 按问题类型筛选
python run_eval.py --question-type user_rewrite

# 使用多个 Judge 模型对比
python run_eval.py --judge-models gpt-4o gpt-4.1 o1

# 重复运行 N 次检测一致性
python run_eval.py --repeat 3

# CI 门禁：通过率低于 80% 则返回非零退出码
python run_eval.py --min-pass-rate 80
```
