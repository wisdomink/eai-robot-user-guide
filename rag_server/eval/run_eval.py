...
Uses the production Triage → Support Agent pipeline by default:
  1. Triage Agent — language detection, translation, product routing, query expansion
  2. Support Agent — product-specific RAG agent with file_search
...
# ── Support Agent configs (mirrors production chatkit_handler.py) ────────

_SUPPORT_MODEL_SETTINGS = ModelSettings(store=False, tool_choice="required")

_SUPPORT_AGENT_CONFIGS: dict[str, dict] = {
    "master-ultra": {
        "name": "Master Ultra Support",
        "instructions_file": "master-ultra",
        "vector_store_id": OPENAI_VECTOR_STORE_MASTER_ULTRA_ID or _FALLBACK_VECTOR_STORE_ID,
    },
    "futurist-ultra": {
        "name": "Futurist Ultra Support",
        "instructions_file": "futurist-ultra",
        "vector_store_id": OPENAI_VECTOR_STORE_FUTURIST_ULTRA_ID or _FALLBACK_VECTOR_STORE_ID,
    },
    "aegis-ultra": {
        "name": "Aegis Ultra Support",
        "instructions_file": "aegis-ultra",
        "vector_store_id": OPENAI_VECTOR_STORE_AEGIS_ULTRA_ID or _FALLBACK_VECTOR_STORE_ID,
    },
    "aegis-edu": {
        "name": "Aegis EDU Support",
        "instructions_file": "aegis-edu",
        "vector_store_id": OPENAI_VECTOR_STORE_AEGIS_EDU_ID or _FALLBACK_VECTOR_STORE_ID,
    },
    "general": {
        "name": "General Support",
        "instructions_file": "general",
        "vector_store_id": _FALLBACK_VECTOR_STORE_ID,
    },
}
...
async def _ask_with_triage(question: str, retries: int) -> str:
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            result = await Runner.run(support_agent, input=triage_output.query_text)
