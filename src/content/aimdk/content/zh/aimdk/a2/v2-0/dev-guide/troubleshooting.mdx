...
description: >-
  ...
  x86/orin 工控机 IP，且能够 ping 通。 2. 检查 HTTP URL 是否正确，可参考各个示例脚本或接口文档。 3. 检查对应 port
  是否有服务启动，ssh 连接到 x86 后命令行执行 `lsof -i:xxxx` 可查看对应端口是否有服务启动。 4. 如果报 …
product: a2
version: v2-0
section: dev-guide
slug:
  ...
  - dev-guide
  - troubleshooting
route: /zh/a2/v2-0/dev-guide/troubleshooting
sourceUrl: 'https://open.agibot.com/docs/aimdk/a2/v2_0/dev_guide/10-interface_issues'
order: 42
---
# 10. 接口调用问题排查
...
1.  检查对应 IP 是否为正确的 x86/orin 工控机 IP，且能够 ping 通。
...
5.  ssh 连接上 x86/orin 工控机后，执行 `aima em load-env` （加载 ROS2 环境变量）后使用 ros2 topic 命令查看相关话题是否有模块在正常发送或接收。
...
2.  为什么调用任何动作播放都只会做抬右手动作
    ...
    2.  一个正确的动作路径应该找到对应动作的 .mcap（新版本）或 .db（老版本）文件，传入完整路径，例如
