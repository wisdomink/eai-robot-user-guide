...
ARG VITE_CHAT_MODE=backend
ARG VITE_CHATKIT_API_URL=/chatkit
ARG VITE_CHATKIT_SESSION_URL=/api/chatkit/session
ARG VITE_CHATKIT_DOMAIN_KEY=local-dev
ARG VITE_SEARCH_API_URL=/api/search

ENV VITE_CHAT_MODE=$VITE_CHAT_MODE
ENV VITE_CHATKIT_API_URL=$VITE_CHATKIT_API_URL
ENV VITE_CHATKIT_SESSION_URL=$VITE_CHATKIT_SESSION_URL
